...
public class NoticeService {
	...
	public NoticeService(NoticeRepository noticeRepository, EmployeeRepository employeeRepository,  ModelMapper modelMapper) {
	...
	public Page<NoticeDTO> searchNoticeByOption(int page, String searchOption, String searchKeyword) {
		...
		if (searchOption.equals("title")) {
			...
			log.info("[EmpService] searchNoticeByOption.getContent() : {}", noticeDTOList.getContent());
			...
			Employee findEmpId = employeeRepository.findByEmpId(searchKeyword)
					.orElseThrow(() -> new UserNotFoundException("해당하는 작성자가 없습니다."));
			Page<Notice> noticeList = noticeRepository.findByNoticeWriter(pageable, findEmpId);
			Page<NoticeDTO> noticeDTOList = noticeList.map(notice -> modelMapper.map(notice, NoticeDTO.class));
