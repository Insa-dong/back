...
public class NoticeService {
	...
	public void updateNotice(NoticeDTO noticeDTO, Long empCode) throws IOException {
		...
		if (notice.getNoticeWriter().getEmpCode().equals(empCode)) {
			...
			/*
			 * 수정 시 파일을 첨부했을 경우 - multipartfile은 파일이 없으면 null이 아니라 빈배열을 반환 따라서 null이 아니라는
	...
	public void deleteNotice(Long noticeCode, Long empCode) throws IOException {
		...
		Employee employee = employeeRepository.findById(empCode)
				.orElseThrow(() -> new IllegalArgumentException("해당 코드의 사용자가 없습니다."));
		...
		} else if (employee.getEmpAuthList().get(0).getAuth().getAuthCode().equals("AU0001")) {
			List<File> files = fileRepository.findByNoticeCode(noticeCode);
