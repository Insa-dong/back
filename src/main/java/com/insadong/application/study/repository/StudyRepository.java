...
public interface StudyRepository extends JpaRepository<Study, Long> {
	...
	@Query(value = "SELECT nvl(max(s.studyCount), 0) from Study s right Join s.training t where t.trainingCode IN :trainingCodeList group by t.trainingCode order by t.trainingCode")
